...
class Receiver(object):
    ...
    def insertInbox(self, body):
        query = "INSERT INTO inbox(`query`, `table`, `pk`, `prev_pk`, `type`, `timestamping`, `id_sender`, `sender_name`, `processed_on`, `label`) "
        query += "VALUES (\"%s\", '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (
            body["query"], body["table"], body["pk"], body["prev_pk"], body["type"], body["timestamping"], body["id_sender"], body["sender_name"], body["processed_on"], body["label"]
        )
