...
class Init(object):
  ...
  def addUUIDColumn(self, tableName):
    self.db.execute(
        "ALTER TABLE %s ADD COLUMN `uuid` VARCHAR(36)" % tableName)
    print("[+] %s : Kolom uuid berhasil ditambahkan" % tableName)

  def addLastUpdateColumn(self, tableName):
    self.db.execute(
        "ALTER TABLE %s ADD COLUMN `last_update` DOUBLE" % tableName)
  ...
  def createTriggerAfterInsert(self, tableName, primaryKey, columns):
    ...
    query = 'CREATE TRIGGER `%s` AFTER INSERT ON `%s` FOR EACH ROW\
			INSERT INTO changelog(`query`, `table`, `pk`, `prev_pk`, `type`, `uuid`, `last_update`) VALUES' % (triggerName, tableName)
  ...
  def createTriggerBeforeInsert(self, tableName, primaryKey, columns):
    ...
    self.db.execute("DROP TRIGGER IF EXISTS %s" % triggerName)
    query = 'CREATE TRIGGER `%s` BEFORE INSERT ON `%s` FOR EACH ROW BEGIN IF NEW.uuid IS NULL THEN\
             SET NEW.uuid = UUID(); SET NEW.last_update = UNIX_TIMESTAMP(NOW(3)); END IF; END;' % (triggerName, tableName)
    # print("query:")
    # print(query)
    self.db.execute(query)
    print('[+] %s : Trigger "%s" => berhasil ditambahkan' %
  ...
  def createTriggerAfterUpdate(self, tableName, primaryKey, columns):
    ...
    queryValue += '"), "%s", NEW.%s, OLD.%s, 2, NEW.uuid, NEW.last_update); END IF; END;' % (
        tableName, primaryKey, primaryKey)
    self.db.execute(query+queryValue)
    print('[+] %s : Trigger "%s" => berhasil ditambahkan' %
          (tableName, triggerName))
  ...
  def createTriggerBeforeUpdate(self, tableName, primaryKey, columns):
    ...
    self.db.execute("DROP TRIGGER IF EXISTS %s" % triggerName)
    query = 'CREATE TRIGGER `%s` BEFORE UPDATE ON `%s` FOR EACH ROW BEGIN IF NEW.pk_correction = 0 AND OLD.pk_correction = 0 AND NEW.last_update = OLD.last_update THEN\
             SET NEW.last_update = UNIX_TIMESTAMP(NOW(3)); END IF; END;' % (triggerName, tableName)
    self.db.execute(query)
    print('[+] %s : Trigger "%s" => berhasil ditambahkan' %
          (tableName, triggerName))
